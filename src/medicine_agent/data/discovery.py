...
def discover_data_files(data_dir: Path, safety: SafetyGate) -> list[DataFileRecord]:
    ...
    if not data_dir.exists():
        return [DataFileRecord(path=str(data_dir), file_type="missing", parser_status="missing", warnings=["data directory does not exist"])]

    for path in sorted(p for p in data_dir.rglob("*") if p.is_file()):
        safety.assert_allowed(OperationClass.READ_FILE, path, "inspect input data file metadata")
        ...
        warnings = [] if file_type == "csv" else [f"{file_type} parsing requires an approved optional parser dependency"]
        records.append(
            DataFileRecord(
                path=str(path),
                file_type=file_type,
